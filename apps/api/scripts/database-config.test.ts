...
const pooled = "postgresql://user:secret@ep-test-pooler.us-east-2.aws.neon.tech/dev?sslmode=require";
...
test("rejects missing, placeholder, insecure, wrong-provider and wrong-pooling configuration without leaking values", () => {
  for (const value of [undefined, "secret", pooled.replace("user", "<role>"),
    pooled.replace("sslmode=require", "sslmode=disable"),
    pooled.replace(".neon.tech", ".example.com"), direct]) {
    assert.throws(() => databaseConnection(value, true), (error: Error) => {
